...
/*   Updated: 2021/02/08 15:33:40 by jondeflo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	clear_all(t_main *main)
{
	...
	{
		pthread_mutex_destroy(main->fork[i].mutex);
		free(main->fork[i].mutex);
		i++;
	}
	free(main->fork);
	free(main->phil);
	free(main->thread);
	pthread_mutex_destroy(main->cond->print_lock);
	pthread_mutex_destroy(main->cond->state_lock);
	free(main->cond->print_lock);
	free(main->cond->state_lock);
	free(main->cond);
	free(main);
}
